            ...
            <form method="POST" action="{{ route('maintenances.update', $maintenance->id) }}">
                ...
                <div class="mt-4 ">
                    ...
                    <select name="vehicle_id" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                        <option value="{{old('model', $maintenance->vehicle->model)}}" class="bg-cyan-200">
                            {{ $maintenance->vehicle->model}}
                            {{ $maintenance->vehicle->brand}}
                            {{ $maintenance->vehicle->version}}
                            {{ $maintenance->vehicle->year}}</option>
                        @foreach ($maintenance->user->vehicles->except($maintenance->vehicle->id) as $vehicle)
